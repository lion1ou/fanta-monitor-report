...
// 自动化驱动（Selenium / Puppeteer / Playwright 等）会把 navigator.webdriver 置为 true
// 旧浏览器 / jsdom 没有该属性，用 Boolean 兜底为 false
const isWebdriver = (): boolean => Boolean(window.navigator.webdriver);

// 获取浏览器信息
export const getBrower = (): IBrowser => {
  let browser = '';
  let browserVersion = '';
  const webdriver = isWebdriver();
  const isBot = webdriver;

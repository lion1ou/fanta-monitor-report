...
export const queryEvents = async (pool: Pool, scope: Scope, filter: EventsFilter): Promise<EventsPage> => {
  ...
  const [count, page] = await Promise.all([
    ...
    pool.query<Row>(
      `${cte} SELECT id, track_id, track_time, track_type, page_path, COALESCE(user_id, '') AS user_id, visitor, session_id, device_type, browser, os, bot_verdict, geo_country, geo_province, geo_city, track_data
       FROM scoped ${where} ORDER BY track_time DESC, id DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,

...
GROUP BY page_path ORDER BY pv DESC, path LIMIT $4`
...
GROUP BY page_path ORDER BY sessions DESC, path LIMIT $4`
...
export const queryPages = async (pool: Pool, scope: Scope, limit: number): Promise<PagesStats> => {
  ...
  return { paths: paths.rows, entries: entries.rows, referrers: referrers.rows }
}
